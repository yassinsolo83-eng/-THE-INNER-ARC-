...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function loadWa() {
      try {
        const supabase = getSupabaseBrowserClient()
        const { data } = await supabase.from('platform_settings').select('key, value').eq('key', 'whatsapp_number')
  ...
  return <>
    ...
    {whatsapp && (
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 left-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lg shadow-[#25D366]/30 transition-transform duration-300 hover:scale-110 active:scale-95"
